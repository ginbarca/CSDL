...
<div class="xs">
	<c:url value="/user/them-nv" var="url" />
	<form:form action="${url}" commandName="addNguoiForm" method="post">
		<fieldset>
			...
			<div class="form-group ${status.error ? 'has-error' : ''}">
				<input name="nguoiDTO.nguoiType" type="hidden" value="NV"/>
				<p class="help-block">
			...
			<div class="form-group ${status.error ? 'has-error' : ''}">
				<label class="control-label" for="nhanVienDTO.luong">Lương(*) (Đơn vị: nghìn đồng)</label>
				<form:input type="text" class="form-control1" required="required"
					path="nhanVienDTO.luong" onfocus="removeCss(this);" />
				<p class="help-block">
					<form:errors path="nhanVienDTO.luong" class="control-label"></form:errors>
				</p>
			</div>
